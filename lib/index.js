...
function loadProtobuf(filePath, typeName) {
  const absPath = path.join(__dirname, '../protobuf', filePath);
  return protobuf.loadSync(absPath).lookupType(typeName);
}

// proto
const ConfigProto = loadProto('tensorflow/core/lib/core/config', 'ConfigProto');
